...
async function speak(
  ...
  console.log(
    `Synthesizing: "${text}" using voice: ${options.voice || "default"}...`,
  );
  ...
  try {
    ...
    console.error(
      "Failed. Is the server running? Run 'yappr serve' in another terminal.",
    );
  }
}
...
async function runListen(args: string[]) {
  ...
  const selectedDevice =
    devices.find((d) => d.index === deviceIndex) || devices[0];
  if (!selectedDevice) {
...
async function runChat(args: string[]) {
  const promptIndex = args.findIndex(
    (arg) => !arg.startsWith("-") && arg !== "chat",
  );
  const prompt = args[promptIndex];

  if (!prompt) {
    console.error(
      "Usage: yappr chat <prompt> [--model <model>] [--voice <voice>]",
    );
    process.exit(1);
  }
...
async function runSpeak(args: string[]) {
  const textIndex = args.findIndex(
    (arg) => !arg.startsWith("-") && arg !== "speak",
  );
  const text = args[textIndex];

  if (!text) {
    console.error(
      "Usage: yappr speak <text> [--voice <voice>] [--speed <speed>]",
    );
    process.exit(1);
  }
